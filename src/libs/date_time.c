...
// Days
static const char days[8][10] = {"",         "Monday", "Tuesday",  "Wednesday",
...
const char_t *formatSystemTime(systime_t time, char_t *str) {
  ...
  if (hours > 0) {
    ...
    sprintf(str, "%" PRIu8 "min %02" PRIu8 "s %03" PRIu16 "ms", minutes,
...
const char_t *formatDate(const DateTime *date, char_t *str) {
  ...
  if (date->dayOfWeek) {
    sprintf(str,
            "%s, %s %" PRIu8 ", %" PRIu16 " %02" PRIu8 ":%02" PRIu8
            ":%02" PRIu8,
            days[MIN(date->dayOfWeek, 7)], months[MIN(date->month, 12)],
            date->day, date->year, date->hours, date->minutes, date->seconds);
  } else {
    sprintf(str,
            "%s %" PRIu8 ", %" PRIu16 " %02" PRIu8 ":%02" PRIu8 ":%02" PRIu8,
            months[MIN(date->month, 12)], date->day, date->year, date->hours,
