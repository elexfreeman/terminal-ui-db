 ...
 * duration of handler call). Handler should return nonzero on success, zero on
 * error.
 *
 *                      Returns 0 on success, line number of first error on
 * parse error (doesn't stop on first error), -1 on file open error, or -2 on
 * memory allocation error (only when INI_USE_STACK is zero).
 *                            */
